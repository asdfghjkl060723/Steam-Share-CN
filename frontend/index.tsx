...
function shareLibraryGame(context) {
  ...
  const finish = (deliveries) => {
    ...
    if (sent === 0) {
      ...
      showLibraryToast(reason ? `发送失败：${reason}` : "Steam 无法发送链接");
    } else if (failed === 0) {
      showLibraryToast(`已发送给 ${sent} 位好友`);
    } else {
      showLibraryToast(`已发送给 ${sent} 位；${failed} 位失败`);
    }
  };
...
function addLibraryDetailsShareButton(libraryWindow) {
  ...
  shareButton.setAttribute("aria-label", "分享");
  shareButton.setAttribute("title", "将此游戏分享给你的好友");
