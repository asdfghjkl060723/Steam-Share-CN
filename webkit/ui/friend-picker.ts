...
export class FriendPicker {
  ...
  private async openInternal(context: ShareContext): Promise<FriendShareResult | undefined> {
    ...
    this.preventBackgroundScroll(elements.backdrop);
    ...
    try {
      ...
      elements.error.textContent =
        error instanceof Error ? error.message : "无法打开 Steam 好友与聊天。";
      elements.cancel.textContent = "关闭";
  ...
  private selectRecipients(
    ...
    const message = `在 Steam 上看看 ${context.title}：\n${context.url}`;
    ...
    return new Promise((resolve) => {
      ...
      const updateSendButton = (): void => {
        ...
        elements.count.textContent = `已选择 ${count} 位`;
        elements.send.disabled = count === 0;
        elements.send.textContent = count === 1 ? "发送给 1 位好友" : `发送给 ${count} 位好友`;
      };
      ...
      renderFriends = (): void => {
        ...
        favoritesHeading.textContent = "收藏";
        ...
        if (visibleFriends.length === 0) {
          ...
          empty.textContent = query.length > 0 ? "没有匹配搜索的好友。" : "暂无好友。";
  ...
  private showFriendContextMenu(
    ...
    action.textContent = isFavorite ? "移除收藏" : "添加收藏";
  ...
  private async sendSelected(
    ...
    try {
      ...
      elements.error.textContent =
        error instanceof Error ? error.message : "Steam 无法发送选中的消息。";
      elements.cancel.disabled = false;
      elements.send.disabled = false;
      elements.send.textContent = "重试";
    }
  }
  ...
  private createDialog(context: ShareContext): PickerElements {
    ...
    allFriendsHeading.textContent = "所有好友";
  ...
  private preventBackgroundScroll(backdrop: HTMLDivElement): void {
    const handler = (event: WheelEvent): void => {
      const scrollable = (event.target as HTMLElement)?.closest<HTMLElement>(
        ".steam-share-picker__friends, .steam-share-picker__favorite-tiles",
      );
      if (scrollable) {
    ...
    backdrop.addEventListener("wheel", handler, { passive: false });
  }
  ...
  private installStyles(): void {
    ...
    style.id = STYLE_ID;
    style.textContent = `
      .steam-share-picker-backdrop { position: fixed; z-index: 2147483646; inset: 0; display: grid; place-items: center; padding: 24px; background: rgba(0, 0, 0, .65); backdrop-filter: blur(8px); -webkit-backdrop-filter: blur(8px); }
      .steam-share-picker { --ss-border:#4c6b88; --ss-text:#d6d7d8; --ss-accent:#66c0f4; --ss-muted:#8f98a0; --ss-input-bg:#111d2a; --ss-avatar-bg:#263b4e; --ss-error:#f28b82; --ss-btn-primary:linear-gradient(to right,#47bfff 5%,#1a9fff 95%); --ss-btn-primary-hover:linear-gradient(to right,#6dccff 5%,#35b7ff 95%); --ss-btn-secondary:linear-gradient(to right,#5a6d7c 5%,#3c4e5f 95%); width: min(440px, 100%); height: min(620px, calc(100vh - 48px)); display: flex; flex-direction: column; box-sizing: border-box; overflow: hidden; padding: 22px; border: 1px solid var(--ss-border); background: linear-gradient(180deg, #2b3d50 0%, #172536 100%); box-shadow: 0 12px 35px rgba(0, 0, 0, .7); color: var(--ss-text); font-family: inherit; }
      ...
      .steam-share-picker__search { box-sizing: border-box; width: 100%; margin-bottom: 9px; padding: 9px 10px; border: 1px solid #000; outline: 1px solid var(--ss-border); background: var(--ss-input-bg); color: #fff; font-family: inherit; }
      .steam-share-picker__count { min-height: 18px; margin-bottom: 6px; color: var(--ss-muted); font-size: 12px; }
      ...
      .steam-share-picker__favorite-tiles { display: flex; flex-wrap: wrap; gap: 10px 12px; min-height: 78px; padding: 0 8px 10px; overscroll-behavior: contain; }
      .steam-share-picker__favorites-hint { align-self: center; padding: 13px 0; color: var(--ss-muted); font-size: 12px; font-style: italic; }
      .steam-share-picker__favorite-tile { display: grid; justify-items: center; width: 64px; padding: 0; border: 0; background: transparent; color: var(--ss-text); cursor: pointer; font-family: inherit; }
      .steam-share-picker__favorite-tile:focus { outline: 0; }
      .steam-share-picker__favorite-tile--dragging { opacity: .42; }
      .steam-share-picker__favorite-avatar { width: 56px; height: 56px; border: 1px solid rgba(255,255,255,.2); background: var(--ss-avatar-bg); object-fit: cover; transform: scale(1); transition: transform 175ms ease, border-color 175ms ease, box-shadow 175ms ease; }
      ...
      .steam-share-picker__favorite-avatar--selected { border-color: var(--ss-accent); box-shadow: 0 0 9px rgba(102,192,244,.85), 0 0 18px rgba(102,192,244,.34); transform: scale(1.07); }
      .steam-share-picker__favorite-name { width: 100%; overflow: hidden; margin-top: 5px; color: var(--ss-text); font-size: 12px; line-height: 15px; text-align: center; text-overflow: ellipsis; white-space: nowrap; }
      .steam-share-picker__friends { flex: 1 1 120px; min-height: 120px; overflow-y: auto; overscroll-behavior: contain; border-top: 1px solid rgba(255,255,255,.08); border-bottom: 1px solid rgba(255,255,255,.08); transition: border-color 160ms ease, box-shadow 160ms ease, background 160ms ease; }
      ...
      .steam-share-picker__avatar { width: 32px; height: 32px; flex: 0 0 32px; border: 1px solid rgba(255, 255, 255, .18); background: var(--ss-avatar-bg); object-fit: cover; transform: scale(1); transition: transform 175ms ease, border-color 175ms ease, box-shadow 175ms ease; }
      .steam-share-picker__avatar--selected { border-color: var(--ss-accent); box-shadow: 0 0 7px rgba(102,192,244,.75), 0 0 14px rgba(102,192,244,.28); transform: scale(1.07); }
      ...
      .steam-share-picker__friend strong { overflow: hidden; color: var(--ss-text); font-size: 14px; font-weight: normal; text-overflow: ellipsis; white-space: nowrap; }
      ...
      .steam-share-picker__friend small { overflow: hidden; margin-top: 2px; color: var(--ss-muted); font-size: 12px; text-overflow: ellipsis; white-space: nowrap; }
      .steam-share-picker__context-menu { position: fixed; z-index: 2147483647; min-width: 186px; padding: 4px; border: 1px solid #5a7b99; background: linear-gradient(180deg, #2b3d50 0%, #182736 100%); box-shadow: 0 5px 18px rgba(0, 0, 0, .65); font-family: inherit; }
      .steam-share-picker__context-action { display: block; width: 100%; padding: 8px 10px; border: 0; background: transparent; color: var(--ss-text); cursor: pointer; font: inherit; font-size: 13px; text-align: left; }
      ...
      .steam-share-picker__empty { padding: 30px 12px; color: var(--ss-muted); text-align: center; }
      .steam-share-picker__error { min-height: 18px; margin-top: 10px; color: var(--ss-error); font-size: 12px; }
      .steam-share-picker__actions { display: flex; justify-content: flex-end; gap: 8px; margin-top: 6px; }
      .steam-share-picker__button { min-width: 112px; padding: 8px 14px; border: 0; background: var(--ss-btn-primary); color: #fff; cursor: pointer; font-family: inherit; font-size: 13px; }
      .steam-share-picker__button:hover:not(:disabled) { background: var(--ss-btn-primary-hover); }
      .steam-share-picker__button:disabled { cursor: default; opacity: .45; }
      .steam-share-picker__button--secondary { background: var(--ss-btn-secondary); }
    `;
